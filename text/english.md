...
James discussed word order under the heading *construction* [@james1847: 133-135], just like Becker [@becker1830: 250-265]. In contrast, in his German writing, Becker originally used the term *Topik* and later *Wortfolge*. In Becker's analysis of German for an English-speaking audience, he proposes a template-style structure for the German sentence ordering with specific slots ordered as columns, cf. [@fig:becker1830_261]. This kind of column-based structure for the analysis of the sentence is adapted by James for English, as shown in [@fig:james1847_134].
...
::: {.im #morell1884_52}
Analysis of a complex sentence by Pope [@last] as a combined scheme from Morell [-@morell1857a: 52], cited here from an identical later version [@morell1884: 52]. This analysis is a combination of Morell's earlier approaches, shown in [@fig:morell1852_25] and [@fig:morell1852_45]. The complex sentence is analysed as five clauses labelled *a* through *e*, each of which is internally analysed. The subordination structure is indicate by reference to the lowercase letters.
:::

In *A grammar of the English language* Morell further develops his proposals into a set of three schemes of increasing detail for the analysis of complex sentences [@morell1857: 91-103]. The first scheme is the basic subordination structure by using lowercase letters, as shown in [@fig:morell1852_25]. The second scheme is the combined table from the *Essentials of English grammar*, as shown in [@fig:morell1884_52]. Additionally, he now adds a third scheme, which is much closer to the original proposals by Becker (cf. [@fig:becker1829_xvii]), separating syntactic parts of the sentence and numbering them, as shown in [@fig:morell1857_93]. Oddly, Morell is not very consistent in the usage of the numbers for the encoding of the relative modification and subordination. Additionally, the parts of the sentence that are separated and numbers are a strange mix of different kinds of structures.
...
All of Greene's textbooks repeat basically the same approach to grammatical analysis for different levels of proficiency and different audiences. The major developments of his approach to graphical grammar are found in the *treatise* [@greene1846], the *grammar* [@greene1860] and the *analysis* [@greene1874]. His other grammatical writings will not be further discussed here.

- *A treatise on the structure of the English language* [@greene1848]^[There is some confusion about the date of publication of the *treatise*. Lyman [-@lyman1922: 7] dates it to 1847, while Görlach dates the original edition to 1846 [@gorlach1998: 152]. A google books copy is also dated 1846, available at <https://books.google.com/?id=GMYCAAAAIAAJ>, accessed 17 July 2026. This date probably arose because the act-of-congress notice at the start of the book is dated 1846. However, the preface is signed by Greene in 1847, and the copy at the library of congress [@greene1848] clearly states a publication date of 1848.]
- *First lessons in language* [@greene1848a]
- *The elements of English grammar* [@greene1856, first edition from 1852]
- *An introduction to the study of English grammar* [@greene1856a; revised -@greene1868]
...
This two-step analysis if taken over by Greene. In his analysis, there are two "principal elements" (subject, predicate) and three "subordinate elements" (adjective, object, adverb) in each sentence [@greene1848: 10]. The five elements are summarised in [@fig:greene1849_70], clearly showing the two steps of his analysis. The individual elements can themselves be modified further, a process that "may be continued indefinitely, forming different degrees of subordination" [@greene1848: 70]. He claims that each of the five elements of a sentence can be of one of three "classes", namely: (1) a word, (2) a phrase or (3) a clause [@greene1848: 67, 82, 111, 120, 162, 167, 184]. He does not give clear examples for each possible combination, unfortunately.

::: {.im #greene1849_70}
*Formula* for a sentence from Greene [-@greene1848: 70], reproduced here from a later identical edition because of better image quality [@greene1849: 70]. There are two different levels of analysis. Each sentence as a whole always consists of a (complex) subject and a predicate. These can be further analysed into five different elements, which can then be further modified. 
:::

In the *grammar of the English language* [@greene1860] he repeats this approach to the analysis of the sentence, but adds a slighly changed graphical approach to their representation, as shown in [@fig:greene1860_172] for the example [@next]. The head words of the subject and predicate are place in the middle, with the modifiers of the subject placed to the left and the modifiers of the predicate are placed to the right [@greene1860: 171-177]. Although this example has a subordinate clause, Greene does not use his notation to further analyse this clause.

::: ex
Greene [-@greene1860: 171], citing Thomas Macaulay (1842) *Frederic the Great*:

Scarcely HAD the RETREAT of Mr. Pitt DEPRIVED Prussia of her only friend, when the death of Elizabeth produced an entire revolution in the politics of the north.
:::
...
::: ex
Greene [-@greene1874: 285], citing William Prescott (1843) *The Conquest of Mexico*:
...
::: {.im #greene1874_288}
Analysis of a paragraph with the *system of significant symbols* for [@last] from Greene [-@greene1874: 288]. It becomes sort-of a puzzle to relate the symbols to the actual parts of the example sentence.
:::

As becomes apparent when attempting to follow the example in [@fig:greene1874_288], this "system of significant symbols" quickly becomes highly convoluted and impractical. Identifying the correspondences between the text and the letters in the formula turns into a puzzle. Moreover, the details of Greene's analysis are often inelegant and difficult to follow. Modifiers are sometimes represented explicitly, while at other times they are simply subsumed under a larger phrase, as indicated by the superscript notation. Non-finite *to*-infinitival clauses are treated without further distinction as objects. I have attempted to make the analysis of the first sentence more explicit below:
...
- **P**: *received*
  ...
  - **.** : *with*
    - **v²**: *the air*
      - **a'**: *embarrassed* 
      - **a²**: *of one*
      - **a³**: *by whom*^[The superscript should probably be "2" here, although it is strange that the same error is repeated so blatently.]
  - **s**: *it*
  - **p²**: *was unexpected*
    - **v'**: *althogether*
    - **(v²^*tr.*^)**: back-reference to the phrase in parenthesis, viz *by whom*
...
- *The English Syntascope* [@brown1839], with separate large leaves [@brown1839a]
...
The pictures below the puzzle pieces in [@fig:brown1839_plate] appear to refer to different kind of example sentences, which are likewise never clearly spelled out. In Brown [-@brown1839: 94] I found the examples sentence [@next a], which seems to correspond roughly to some of these pictures. Another approach to interlocking pieces is shown in [@fig:brown1839_105], illustrating the structure of two separate sentences [@next a,b]. The lower-case letters do not represent any parts of speech, but appear simply to be used as way to refer to the different parts of the illustration. Notewhorthy is the hierarchical interlocking structure for the notion of degree/rank, as discussed above with [@fig:brown1826_36].

::: ex
- If this man's bird was shot by John.
- John walks very much too fast.
- Nathanael reads books.
:::

::: {.im #brown1839_plate}
Syntactic analysis using puzzle pieces from Brown [-@brown1839a]. Each puzzle piece represents a word, and the capital letters are roughly referring to parts of speech. However, the precise intention is never spelled out by Brown. The pictures below the puzzle pieces seem to refer to specific example sentences, one of the them might be [@last a].
:::
